...
declaraciones:
    INT ID PUNTOYCOMA { agregar_tipo($2, 1); }  // 1 representa tipo INT
    | declaraciones INT ID PUNTOYCOMA { agregar_tipo($3, 1); }
    ;

asignaciones:
    ID IGUAL ID PUNTOYCOMA {
        if (!existe($1) || !existe($3))
            printf("Error: identificador no encontrado\n");
